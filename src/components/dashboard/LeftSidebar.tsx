...
interface LeftSidebarProps {
  ...
  onImportPrompts: (importedData: Prompt[]) => void;
}
...
export const LeftSidebar = ({
  ...
  const fetchArchievedPrompts = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        const formattedPrompts = response.data.data
          .map((p: BackendPrompt) => ({
            id: p._id,
            title: p.title,
          ...
          .sort(
            (a: BackendPrompt, b: BackendPrompt) =>
              new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
          );
        setArchivedPrompts(formattedPrompts);
      } else {
        console.error(
          "Failed to fetch archived prompts:",
          response.data.message
        );
        toast.error(
          response.data.message || "Failed to fetch archived prompts."
        );
      }
    } catch (error) {
  ...
  const restorePrompt = async (promptId: string) => {
    try {
      ...
      if (response.data.success) {
        ...
        console.log(
          `✅ Prompt ${promptId} restored successfully.`,
          response.data
        );
  ...
  const handleExportPrompts = async () => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/data/export`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      try {
        ...
        if (Array.isArray(jsonData)) {
          jsonData = jsonData.map((prompt, promptIndex) => ({
            ...prompt,
            id: `imported-prompt-${promptIndex}`,
            versions:
              prompt.versions?.map(
                (version: ImportedVersion, versionIndex: number) => ({
                  ...version,
                  id: `imported-version-${promptIndex}-${versionIndex}`,
    ...
    >
        ...
        <div className="flex items-center justify-between gap-2">
          <Button
            ...
            className={`flex-1 justify-start text-sm font-medium ${
              viewMode === "active"
                ? "text-emerald-600 bg-emerald-100/50 hover:bg-emerald-100/70"
          ...
          >
            <MessageSquare
              className={`w-4 h-4 mr-2 ${
                viewMode === "active" ? "text-emerald-600" : "text-emerald-500"
              }`}
            />
            Active
          </Button>
          <Button
            ...
            className={`flex-1 justify-start text-sm font-medium ${
              viewMode === "archived"
                ? "text-red-600 bg-red-100/50 hover:bg-red-100/70"
          ...
          >
            <Archive
              className={`w-4 h-4 mr-2 ${
                viewMode === "archived" ? "text-red-600" : "text-gray-500"
              }`}
            />
            Archived
          </Button>
      ...
      >
        {isLoadingArchived && viewMode === "archived" ? (
          <div className="p-3 text-center text-emerald-500">
            Loading archived prompts...
          </div>
          ...
          promptsToDisplay.map((prompt) => (
            <motion.div
              ...
              whileTap={{ scale: 0.98 }}
              className={`p-3 mb-2 rounded-lg cursor-pointer transition-all flex items-center justify-between ${
                selectedPromptInView?.id === prompt.id
            ...
            >
              ...
              >
                ...
                <div className="flex gap-4 min-w-0 flex-grow">
                  ...
                  <p className="text-xs text-emerald-600/70 mt-0.5 whitespace-nowrap">
                    {" "}
              ...
              {viewMode === "active" ? (
                ...
                >
                  ...
                  >
                    <path d="M12 2v10l4-4m-4 4-4-4M4 12v10a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-10"></path>
                  </svg>
                </Button>
              )}
            </motion.div>
      ...
      <div className="p-4 border-t border-emerald-200/50">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-emerald-900">{username}</p>
            <p className="text-xs text-emerald-600/70">{email}</p>
          </div>
        </div>
